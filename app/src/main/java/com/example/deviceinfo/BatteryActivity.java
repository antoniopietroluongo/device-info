...
public class BatteryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        customAdapter = new CustomAdapter(batteryData());
    ...
    public void task() {
        ...
        timerTask = new TimerTask() {
            ...
            public void run() {
                runOnUiThread(() -> {
                    customAdapter.setData(batteryData());
    ...
    private List<Item> batteryData() {
        List<Item> data = new ArrayList<>();
        BatteryManager batteryManager = (BatteryManager) getApplicationContext()
                .getSystemService(Context.BATTERY_SERVICE);
        Intent intent = registerReceiver(null, new IntentFilter(Intent.ACTION_BATTERY_CHANGED));
        String technology = intent.getStringExtra(BatteryManager.EXTRA_TECHNOLOGY);
        data.add(new Item(R.layout.two_line, getString(R.string.battery_level), batteryManager.getIntProperty(BatteryManager.BATTERY_PROPERTY_CAPACITY) + "%"));
        data.add(new Item(R.layout.two_line, getString(R.string.battery_power_source), pluggedFromIntent(intent.getIntExtra(BatteryManager.EXTRA_PLUGGED, -1))));
        data.add(new Item(R.layout.two_line, getString(R.string.battery_status), statusFromIntent(intent.getIntExtra(BatteryManager.EXTRA_STATUS, -1))));
        data.add(new Item(R.layout.two_line, getString(R.string.battery_current_charge), batteryManager.getIntProperty(BatteryManager.BATTERY_PROPERTY_CHARGE_COUNTER) / 1000 + " mAh"));
        data.add(new Item(R.layout.two_line, getString(R.string.battery_health), healthFromIntent(intent.getIntExtra(BatteryManager.EXTRA_HEALTH, -1))));
        data.add(new Item(R.layout.two_line, getString(R.string.battery_technology), technology));
        data.add(new Item(R.layout.two_line, getString(R.string.battery_temperature), intent.getIntExtra(BatteryManager.EXTRA_TEMPERATURE, -1) / 10.0 + " °C"));
        data.add(new Item(R.layout.two_line, getString(R.string.battery_voltage), intent.getIntExtra(BatteryManager.EXTRA_VOLTAGE, -1) / 1000.0 + " V"));
        return data;
    }
